...
#### 变动成本（含附加费用）
```
变动成本 = Σ(每小时下网负荷 × 综合电价) / 10000

综合电价 = 下网电价 + 输配电费 + 线损费用 + 系统运行费 + 政府性基金

其中：
- 下网电价：从 CSV 数据导入的每小时下网电价（元/kWh）
- 输配电费：默认 0.0486 元/kWh
- 线损费用：默认 0.017 元/kWh
- 系统运行费：默认 0.065 元/kWh
- 政府性基金：默认 0.0041 元/kWh
```

#### 下网度电成本
```
下网度电成本 = 下网总成本 × 10000 / 总下网电量
...
#### 光伏总成本
```
光伏总成本 = 投资年度折旧 + 备份费和政府基金总额

备份费和政府基金总额 = 总光伏电量 × 备份费单价 / 10000
```

#### 光伏度电成本
```
光伏度电成本 = 光伏总成本 × 10000 / 总光伏电量
```
...
#### 风电总成本
```
风电总成本 = 投资年度折旧 + 备份费和政府基金总额

备份费和政府基金总额 = 总风电电量 × 备份费单价 / 10000
```

#### 风电度电成本
```
风电度电成本 = 风电总成本 × 10000 / 总风电电量
```
...
#### a. 燃料成本
```
燃料成本 = Σ(每小时火电出力 × 火电出力单价) / 10000

火电出力单价 = 度电煤耗 × 入炉煤综合单价 / 1000000 + 政府性基金 + 政策性交叉补贴 + 备容费
...
最大出力 = 调峰机组最大出力 + 热定电机组基础电出力

备容费计算：
- 单价计费模式：备容费 = 备容费单价（元/kWh）
- 总价计费模式：备容费 = 备容费总额 × 10000 / 总火电电量（元/kWh）

参数默认值：
- 政府性基金：0.0241 元/kWh
- 政策性交叉补贴：0.0128 元/kWh
- 备容费单价：0.05 元/kWh
- 备容费总额：0.0 万元
- 入炉煤综合单价：162.4 元/吨
- 基准煤耗：500.0 g/kWh
...
碳配额 = 火电总发电量 × 碳排放分配系数 / 1000

实际碳排放量 = 碳排放量 - 碳配额

碳排放费用 = 实际碳排放量 × 碳排放单价 / 10000
...
#### d. 绿证费用
```
绿电实际占比 = (光伏实际发电量 + 风电实际发电量) / 总用电量

总用电量 = 火电总发电量 + 光伏总发电量 + 风电总发电量 + 下网总负荷

绿证缺口 = max(绿色能源占比 - 绿电实际占比, 0)

绿证费用 = 绿证缺口 × 总用电量 × 绿证单价 / 10000
...
其他可变成本 = 总发电量 × 其他可变成本单价 / 10000

总发电量 = 火电总发电量 + 光伏总发电量 + 风电总发电量
```

#### 其他度电成本
```
其他度电成本 = 其他总成本 × 10000 / 总发电量
```
...
#### 年度总成本
```
年度总成本 = 下网总成本 + 光伏总成本 + 风电总成本 + 火电总成本 + 其他总成本
```
